...
from extended_data_types.cli.commands.hcl2 import (convert, format, hcl2,
                                                   merge, validate)
...
def test_merge_files(runner: CliRunner, tmp_path: Path, sample_tf: Path, sample_variables: Path):
    """Test merging multiple Terraform files."""
    output_file = tmp_path / "merged.tf"
    
    result = runner.invoke(merge, [
        str(sample_tf),
        str(sample_variables),
...
def test_convert_with_invalid_format(runner: CliRunner, tmp_path: Path, sample_tf: Path):
