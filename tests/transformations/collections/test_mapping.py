...
def test_merge_dicts() -> None:
    ...
    d1 = {"a": 1, "b": {"x": 1, "y": 2}}
    d2 = {"b": {"y": 3, "z": 4}, "c": 5}
    assert merge_dicts(d1, d2) == {"a": 1, "b": {"x": 1, "y": 3, "z": 4}, "c": 5}
...
def test_flatten_dict() -> None:
    ...
    d = {"a": 1, "b": {"x": 2, "y": 3}, "c": 4}
    assert flatten_dict(d) == {"a": 1, "b.x": 2, "b.y": 3, "c": 4}

    # Test with custom separator
    assert flatten_dict(d, separator="/") == {"a": 1, "b/x": 2, "b/y": 3, "c": 4}
    ...
    # Test with lists
    d = {"a": [1, 2, {"b": 3}]}
    assert flatten_dict(d) == {"a.0": 1, "a.1": 2, "a.2.b": 3}
...
def test_unflatten_dict() -> None:
    ...
    assert unflatten_dict(d) == {"a": 1, "b": {"x": 2, "y": 3}, "c": 4}

    # Test with custom separator
    d = {"a": 1, "b/x": 2, "b/y": 3, "c": 4}
    assert unflatten_dict(d, separator="/") == {"a": 1, "b": {"x": 2, "y": 3}, "c": 4}

    # Test with array indices
    d = {"a.0": 1, "a.1": 2, "a.2.b": 3}
    assert unflatten_dict(d) == {"a": [1, 2, {"b": 3}]}
...
def test_filter_dict() -> None:
    ...
    assert filter_dict(
        d, key_fn=lambda k: k in ["a", "b", "c"], value_fn=lambda v: v > 2
    ) == {"c": 3}
...
def test_transform_dict() -> None:
    ...
    assert transform_dict(d, key_fn=str.upper) == {"A": 1, "B": 2, "C": 3}

    # Test value transformation
    assert transform_dict(d, value_fn=lambda x: x * 2) == {"a": 2, "b": 4, "c": 6}

    # Test both transformations
    assert transform_dict(d, key_fn=str.upper, value_fn=lambda x: x * 2) == {
        "A": 2,
        "B": 4,
        "C": 6,
    }
