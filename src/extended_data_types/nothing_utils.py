...
def all_non_empty(*vals: Any) -> list[Any]:
    ...
    Returns:
        list[Any]: The list of non-empty values.
    """
...
def any_non_empty(m: Mapping[Any, Any], *keys: Any) -> Mapping[Any, Any]:
    ...
    Returns:
        Mapping[Any, Any]: A mapping containing the first non-empty value.
    """
    for k in keys:
        v = m.get(k)
        if not is_nothing(v):
...
def yield_non_empty(
    ...
    Yields:
        Generator[Mapping[Any, Any], None, None]: A generator yielding non-empty values.
    """
    for k in keys:
        v = m.get(k)
        if not is_nothing(v):
