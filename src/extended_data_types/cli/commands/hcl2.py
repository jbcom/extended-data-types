...
@click.option(
    "--from-format", "-f", default="hcl2", help="Input format (default: hcl2)"
)
...
@click.option(
    ...
    help="Output file (default: merged.tf)",
)
@click.option("--indent", "-i", type=int, default=2, help="Indentation spaces")
@click.option("--sort-keys", "-s", is_flag=True, help="Sort keys alphabetically")
def merge(files: tuple[Path, ...], output: Path | None, indent: int, sort_keys: bool):
