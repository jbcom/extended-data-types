...
def transform_if(
    condition: bool | Callable[[T], bool], func: Transform[T, U]
) -> Transform[T, U | T]:
...
def transform_any(transforms: Sequence[Transform[T, U]]) -> Transform[T, U | None]:
...
def transform_all(transforms: Sequence[Transform[T, Any]]) -> Transform[T, list[Any]]:
