...
from ..core.types import (convert_special_type, strtobool, strtodate,
                          strtodatetime, strtofloat, strtoint, strtopath,
                          strtotime)
from .detection import is_potential_json, is_potential_yaml
...
def reconstruct_from_serialized(obj: Any, fail_silently: bool = False) -> Any:
    ...
    if isinstance(obj, Mapping):
        return {
            k: reconstruct_from_serialized(v, fail_silently)
            for k, v in obj.items()
        }
    if isinstance(obj, (list, set)):
        return type(obj)(
            reconstruct_from_serialized(v, fail_silently)
            for v in obj
        )
    return obj 