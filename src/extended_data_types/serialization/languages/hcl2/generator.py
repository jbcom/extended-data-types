...
from .types import Block, Expression, Function, HCLFile, Reference
...
class HCL2Generator:
    ...
    def generate(self, hcl_file: HCLFile) -> str:
        ...
        if hcl_file.terraform_version or hcl_file.required_providers:
            parts.append(
                self._generate_terraform_block(
                    hcl_file.terraform_version, hcl_file.required_providers
                )
            )

        # Generate all blocks
        blocks = (
            sorted(hcl_file.blocks, key=lambda b: str(b))
            if self.sort_keys
            else hcl_file.blocks
        )
        for block in blocks:
    ...
    def _generate_terraform_block(
        self, version: str | None, providers: dict[str, str]
    ) -> str:
        ...
        if providers:
            parts.append(
                f"{self._indent(1)}required_providers {self._generate_map(providers, 2)}"
            )
    ...
    def _generate_block(self, block: Block, level: int = 0) -> str:
        ...
        if block.meta_args.count is not None:
            parts.append(
                f"{self._indent(level + 1)}count = {self._generate_expression(block.meta_args.count)}"
            )

        if block.meta_args.for_each is not None:
            parts.append(
                f"{self._indent(level + 1)}for_each = {self._generate_expression(block.meta_args.for_each)}"
            )
        ...
        if block.meta_args.lifecycle:
            parts.append(
                self._generate_lifecycle_block(block.meta_args.lifecycle, level + 1)
            )

        # Regular attributes
        attributes = (
            sorted(block.attributes.items())
            if self.sort_keys
            else block.attributes.items()
        )
        for key, value in attributes:
            parts.append(
                f"{self._indent(level + 1)}{key} = {self._generate_expression(value)}"
            )

        # Nested blocks
        blocks = (
            sorted(block.blocks, key=lambda b: str(b))
            if self.sort_keys
            else block.blocks
        )
    ...
    def _generate_map(self, map_data: dict[str, Any], indent_level: int = 1) -> str:
        ...
        for key, value in items:
            parts.append(
                f"{self._indent(indent_level)}{key} = {self._generate_expression(value)}"
            )
    ...
    def _generate_lifecycle_block(self, lifecycle: dict[str, Any], level: int) -> str:
        ...
        for key, value in (
            sorted(lifecycle.items()) if self.sort_keys else lifecycle.items()
        ):
            parts.append(
                f"{self._indent(level + 1)}{key} = {self._generate_expression(value)}"
            )
